...
export function createBearerAuthenticator(config: AppConfig, db: Database) {
  ...
  return async (request: FastifyRequest): Promise<BearerIdentity> => {
    ...
    try {
      verified = await jwtVerify(token, keySet, { issuer: config.issuer.origin });
    } catch {
    ...
    const [client] = await db
      .select({ audience: oauthClients.accessTokenAudience, enabled: oauthClients.enabled })
      .from(oauthClients)
    ...
    return {
      ...
      clientId,
      scope: new Set(
        typeof verified.payload.scope === 'string' ? verified.payload.scope.split(' ') : [],
      ),
