...
function requireCsrf(request: FastifyRequest, token: unknown, config: AppConfig): void {
  if (!verifyCsrf(request, token, config))
...
export async function registerAdminRoutes(
  ...
  app.post<{ Params: { clientId: string }; Body: CsrfBody & { enabled: boolean } }>(
    '/api/v1/admin/clients/:clientId/enabled',
    async (request) => {
      const admin = await requireAdmin(request, sessions);
      requireCsrf(request, request.body.csrfToken, config);
      ...
      await audit.write({
        ...
        success: true,
        actorUserId: admin.user.id,
        metadata: { clientId: request.params.clientId, enabled: request.body.enabled },
        request,
      });
  ...
  app.post<{ Params: { clientId: string }; Body: CsrfBody }>(
    '/api/v1/admin/clients/:clientId/regenerate-secret',
    async (request) => {
      const admin = await requireAdmin(request, sessions);
      requireCsrf(request, request.body.csrfToken, config);
  ...
  app.get<{ Querystring: Record<string, unknown> }>('/api/v1/admin/users', async (request) => {
    ...
    const rows = await db
      ...
      .where(
        search
          ? ilike(users.email, `%${search.replaceAll('%', '\\%').replaceAll('_', '\\_')}%`)
          : undefined,
      )
  ...
  app.post<{ Params: { userId: string }; Body: CsrfBody & { active: boolean } }>(
  ...
  app.post<{ Params: { userId: string }; Body: CsrfBody }>(
    '/api/v1/admin/users/:userId/logout-everywhere',
    async (request) => {
      const admin = await requireAdmin(request, sessions);
      requireCsrf(request, request.body.csrfToken, config);
  ...
  app.delete<{ Params: { userId: string }; Body: CsrfBody & { confirmEmail: string } }>(
    ...
    async (request) => {
      ...
      if (target.email !== request.body.confirmEmail) {
        throw new AppError(
          ...
          'Enter the exact user email to confirm deletion',
        );
      }
      if (admin.user.id === request.params.userId) {
        throw new AppError(
          ...
          'An administrator cannot permanently delete the current account',
        );
      }
      await audit.write({
        type: 'user.deleted',
      ...
      await db
        .delete(users)
        .where(and(eq(users.id, request.params.userId), eq(users.status, 'deactivated')));
      return { deleted: true };
    },
  );
  ...
  app.delete<{ Params: { sessionId: string }; Body: CsrfBody }>(
  ...
  app.patch<{
    ...
    if (request.body.minPasswordLength < 6 || request.body.minPasswordLength > 256) {
      throw new AppError(
        ...
        'Password minimum must be between 6 and 256',
      );
    }
    if (request.body.ssoAbsoluteTtlSeconds < request.body.ssoIdleTtlSeconds) {
      throw new AppError(
        ...
        'Absolute session TTL must not be shorter than idle TTL',
      );
    }
