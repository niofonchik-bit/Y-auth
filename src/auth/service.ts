...
export class AuthService {
  ...
  private async enforceCaptcha(
    ...
    if (!token) {
      await this.audit.write({ type: 'captcha.required', success: false, request });
    ...
    if (!this.captcha || !(await this.captcha.verify(token, request.ip))) {
      await this.audit.write({ type: 'captcha.failed', success: false, request });
  ...
  async captchaRequired(
    ...
    const attempts = await this.rateLimiter.peek(
      kind === 'login' ? 'login-ip' : 'registration-ip',
      ip,
    );
    return attempts >= 3;
  }
  ...
  async register(
    ...
    const registrationLimit = await this.rateLimiter.consume(
      'registration-ip',
      request.ip,
      5,
      3_600,
    ...
    await this.enforceCaptcha(
      policy.captchaMode === 'always_registration' ||
        (policy.captchaMode === 'adaptive' && registrationLimit.remaining <= 2),
      input.captchaToken,
      request,
    );
  ...
  async requestPasswordReset(email: string, request: FastifyRequest): Promise<void> {
    ...
    const [user] = await this.db
      ...
      .limit(1);
    if (!user) {
      await this.audit.write({ type: 'password.reset.requested', success: true, request });
      return;
    }
    ...
    await this.db.transaction(async (tx) => {
      ...
      await tx
        ...
        .values({ userId: user.id, tokenHash: sha256(token), expiresAt });
    });
    await this.audit.write({
  ...
  async completePasswordReset(
    ...
    await this.db.transaction(async (tx) => {
      await tx
        ...
        .where(eq(passwordCredentials.userId, reset.user.id));
      await tx
        .update(passwordResetTokens)
        .set({ usedAt: new Date() })
        .where(eq(passwordResetTokens.id, reset.token.id));
      await tx
        .update(users)
        .set({ sessionVersion: reset.user.sessionVersion + 1, updatedAt: new Date() })
