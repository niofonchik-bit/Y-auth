...
export async function registerAuthRoutes(
  ...
  app.post<{ Body: DirectLoginBody }>(
    ...
    async (request, reply) => {
      ...
      return {
        user: { id: result.user.id, email: result.user.email, isAdmin: result.user.isAdmin },
      };
    },
  );
    ...
    '/api/v1/auth/password-reset/request',
    {
      schema: {
        body: {
          ...
          additionalProperties: false,
          properties: { email: { type: 'string', maxLength: 320 }, csrfToken: { type: 'string' } },
        },
      },
    },
    async (request) => {
  ...
  app.post<{ Body: ResetCompleteBody }>(
    ...
    async (request) => {
      ...
      await dependencies.auth.completePasswordReset(
        request.body.token,
