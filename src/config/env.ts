...
function parseJwks(value: string): AppConfig['jwks'] {
	...
	const result = z.object({ keys: z.array(z.record(z.string(), z.unknown())).min(1) }).safeParse(parsed);
	if (!result.success) {
...
export function loadConfig(environment?: NodeJS.ProcessEnv): AppConfig {
	...
	const raw = schema.parse(environment ?? process.env);
	...
	if (oidcCookieKeys.length < 2 || oidcCookieKeys.some((value) => value.length < MIN_SECRET_BYTES)) {
	...
	if (isProduction && raw.CAPTCHA_MODE !== 'off' && (!raw.TURNSTILE_SECRET_KEY || !raw.TURNSTILE_SITE_KEY)) {
		throw new Error('TURNSTILE_SITE_KEY and TURNSTILE_SECRET_KEY are required when CAPTCHA is enabled in production');
	}
