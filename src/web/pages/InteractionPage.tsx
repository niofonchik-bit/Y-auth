...
export default function InteractionPage() {
    ...
    api<Interaction>(`/api/v1/interactions/${encodeURIComponent(uid)}`, {
      ...
      .catch((reason: unknown) => {
        if (!controller.signal.aborted)
          setError(
            reason instanceof Error ? reason.message : 'Unable to load authorization request',
          );
      });
  ...
  const captchaRequired = isRegistration
    ? interaction.registrationCaptchaRequired
    : interaction.loginCaptchaRequired;
    ...
    return (
      <Paper sx={{ maxWidth: 480, mx: 'auto', p: 4 }}>
        ...
        <Typography color="text.secondary">
          This application requests access to your basic Y.auth profile.
        </Typography>
        ...
        {isRegistration && !interaction.registrationEnabled ? (
          ...
          >
            ...
            <Stack spacing={2.5}>
              ...
              {isRegistration && (
                <TextField name="displayName" label="Display name (optional)" autoComplete="name" />
              )}
              <TextField
                name="email"
                label="Email"
              ...
              <TextField
                ...
                slotProps={{
                  htmlInput: {
                    maxLength: 256,
                    ...(isRegistration ? { minLength: interaction.minPasswordLength } : {}),
                  },
                }}
                autoComplete={isRegistration ? 'new-password' : 'current-password'}
                helperText={
                  isRegistration ? `Minimum ${interaction.minPasswordLength} characters` : undefined
                }
              />
              {captchaRequired && interaction.turnstileSiteKey && (
                <Turnstile siteKey={interaction.turnstileSiteKey} />
              )}
              {captchaRequired && !interaction.turnstileSiteKey && (
