...
export default function AccountPage() {
	...
	useEffect(() => {
		load().catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Unable to load account'));
	}, [load]);
	...
	if (!account) return <Alert severity={error ? 'warning' : 'info'}>{error ?? 'Loading account…'}</Alert>;
		...
		<Stack spacing={3}>
			...
			<Paper sx={{ p: 3 }}>
				...
				<Stack divider={<Divider flexItem />}>
					{sessions.map((session) => (
						<Stack key={session.id} direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ py: 2, justifyContent: 'space-between' }}>
							<div>
								<Typography>
									{session.browser} · {session.os} {session.current && <Chip size="small" label="Current" color="primary" />}
								</Typography>
			...
			<Paper sx={{ p: 3, borderColor: 'error.main' }} variant="outlined">
				...
				<Typography color="text.secondary" sx={{ my: 1 }}>
					Your sessions and application grants will be revoked. Data is retained for later recovery or cleanup.
				</Typography>
				<Button
					color="error"
					disabled={loading}
