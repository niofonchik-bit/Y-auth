...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    csrfToken()
      .then(setCsrf)
      .catch(() => setMessage({ type: 'error', text: 'Unable to initialize secure form' }));
  }, []);
    ...
    try {
      if (token) {
        await api('/api/v1/auth/password-reset/complete', {
          method: 'POST',
          body: JSON.stringify({ token, password: form.get('password'), csrfToken: csrf }),
        });
        ...
        const result = await api<{ message: string }>('/api/v1/auth/password-reset/request', {
          method: 'POST',
          body: JSON.stringify({ email: form.get('email'), csrfToken: csrf }),
        });
