...
export default function AdminPage() {
	...
	const load = useCallback(async () => {
		const [dashboardResult, clientResult, userResult, sessionResult, auditResult, settingsResult, token] = await Promise.all([
	...
	useEffect(() => {
		load().catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Unable to load admin data'));
	}, [load]);
	...
	const createClient = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			setCreateOpen(false);
			setSecret(created.clientSecret ? `${created.clientId}\n${created.clientSecret}` : created.clientId);
			await load();
		} catch (reason) {
	...
	return (
		<Stack spacing={3}>
			...
			{tab === 1 && (
				<Stack spacing={2}>
					<Button variant="contained" startIcon={<Add />} sx={{ alignSelf: 'flex-start' }} onClick={() => setCreateOpen(true)}>
						Create client
					</Button>
					{clients.map((client) => (
								...
								<Stack direction="row" spacing={1}>
									...
									{client.type === 'confidential' && (
										<Button
											...
											disabled={loading}
											onClick={() =>
												window.confirm('Regenerate this secret? The previous value will stop working immediately.') &&
												run(async () => {
								...
								<Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }}>
									...
									{user.status === 'deactivated' && (
										<Button
											...
											disabled={loading}
											onClick={() => {
												const confirmation = window.prompt(`Enter ${user.email} to permanently delete this user.`);
						...
						<Paper key={session.id} sx={{ p: 3 }}>
							<Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ justifyContent: 'space-between' }}>
								<div>
									...
									<Typography variant="body2" color="text.secondary">
										{session.lastIp} · {new Date(session.lastSeenAt).toLocaleString()} · {session.userAgent}
									</Typography>
								</div>
								{!session.revokedAt && (
									<Button
				...
				<Stack spacing={1}>
					{events.map((event) => (
						<Paper key={event.id} sx={{ p: 2 }}>
							...
							<Typography variant="body2" color="text.secondary">
								{new Date(event.createdAt).toLocaleString()} {event.reasonCode ? `· ${event.reasonCode}` : ''}
							</Typography>
						</Paper>
					))}
				</Stack>
			...
			{tab === 5 && settings && (
				<Paper component="form" onSubmit={saveSettings} sx={{ p: 3 }}>
					<Stack spacing={2} sx={{ maxWidth: 560 }}>
						<FormControlLabel
							control={<Checkbox name="registrationEnabled" defaultChecked={settings.registrationEnabled} />}
						...
						/>
						<TextField name="captchaMode" label="CAPTCHA mode" select defaultValue={settings.captchaMode}>
						...
						/>
						<TextField name="ssoIdleTtlSeconds" label="SSO idle TTL (seconds)" type="number" defaultValue={settings.ssoIdleTtlSeconds} />
						<TextField
			...
			<Dialog open={createOpen} onClose={() => !loading && setCreateOpen(false)} fullWidth maxWidth="sm">
			...
			<Dialog open={secret !== null} onClose={() => setSecret(null)} fullWidth maxWidth="sm">
				...
				<DialogContent>
					...
					<TextField fullWidth multiline value={secret ?? ''} slotProps={{ htmlInput: { readOnly: true } }} />
				</DialogContent>
				<DialogActions>
