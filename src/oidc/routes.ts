...
async function acceptFirstPartyConsent(
  provider: Provider,
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<boolean> {
  ...
  if (!grant) grant = new provider.Grant({ accountId: details.session.accountId, clientId });
...
export async function registerOidcRoutes(
    ...
    if (details.prompt.name === 'login') {
      ...
      if (active) {
        await finish(provider, request, reply, {
          login: { accountId: active.user.id, remember: true, amr: ['pwd'] },
        });
        return;
      }
    }
  ...
  app.post<{ Params: InteractionParams; Body: InteractionForm }>(
    ...
    async (request, reply) => {
      ...
      if (details.prompt.name !== 'login') {
        throw new AppError(
          ...
          'Login is not expected for this interaction',
        );
      }
      ...
      try {
        ...
        if (error instanceof AppError) {
          return reply.redirect(
            `/interaction/${encodeURIComponent(request.params.uid)}?interactionError=${encodeURIComponent(error.code)}`,
          );
        }
        throw error;
      }
      await finish(provider, request, reply, {
        login: { accountId: result.user.id, remember: true, amr: ['pwd'] },
      });
    },
  );
    ...
    async (request, reply) => {
      ...
      if (details.prompt.name !== 'create' && details.prompt.name !== 'login') {
        throw new AppError(
          409,
          'INTERACTION_MISMATCH',
          'Registration is not expected for this interaction',
        );
      }
      const clientId = stringValue(details.params.client_id);
  ...
  app.use((request, response, next) => {
    ...
    if (config.isProduction && incomingHostname !== config.issuer.hostname) {
      ...
      response.end(
        JSON.stringify({ error: 'invalid_request', error_description: 'Invalid request host' }),
      );
      return;
    }
    ...
    const run = async () => {
      if (path === '/oauth/token') {
        const forwarded =
          config.trustProxy === false ? undefined : request.headers['x-forwarded-for'];
        const ip =
        ...
        if (authorization?.startsWith('Basic ')) {
          try {
            clientId =
              Buffer.from(authorization.slice(6), 'base64').toString('utf8').split(':')[0] ||
              'confidential';
          } catch {
