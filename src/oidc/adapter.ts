  ...
  return class PostgresOidcAdapter implements Adapter {
    ...
    async upsert(id: string, payload: AdapterPayload, expiresIn?: number): Promise<void> {
      ...
      if (
        ...
        const [user] = await db
          ...
          .from(users)
          .where(eq(users.id, payload.accountId))
          .limit(1);
        if (user?.status === 'deactivated')
          throw new Error('Cannot issue an artifact for a deactivated user');
      }
    ...
    async find(id: string): Promise<AdapterPayload | undefined> {
      const [record] = await db
        ...
        .limit(1);
      if (!record || (this.model === 'Client' && record.payload.y_auth_enabled === false))
        return undefined;
      return record.consumedAt
        ? { ...record.payload, consumed: Math.floor(record.consumedAt.getTime() / 1_000) }
        : record.payload;
    }
