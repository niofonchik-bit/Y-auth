...
function describeUserAgent(userAgent: string) {
  ...
  return {
    browser:
      [result.browser.name, result.browser.version].filter(Boolean).join(' ') || 'Unknown browser',
...
export class SessionService {
  ...
  async authenticate(request: FastifyRequest): Promise<AuthenticatedSession | null> {
    ...
    const activityThreshold = new Date(
      now.getTime() - this.config.SESSION_ACTIVITY_INTERVAL_SECONDS * 1_000,
    );
    if (result.session.lastSeenAt < activityThreshold) {
      const idleExpiry = new Date(
        Math.min(
          now.getTime() + this.config.SSO_IDLE_TTL_SECONDS * 1_000,
          result.session.absoluteExpiresAt.getTime(),
        ),
      );
      await this.db
        .update(userSessions)
        .set({ lastSeenAt: now, lastIp: request.ip, expiresAt: idleExpiry })
        .where(
          and(
            eq(userSessions.id, result.session.id),
            lt(userSessions.lastSeenAt, activityThreshold),
          ),
        );
    }
    return result;
  ...
  async list(userId: string) {
    ...
    return rows.map((row) => ({ ...row, ...describeUserAgent(row.userAgent) }));
  }
  ...
  async revoke(userId: string, sessionId: string, reason: string): Promise<void> {
    ...
    const grants = await this.db
      .select({ grantId: sessionClients.grantId })
      .from(sessionClients)
    ...
    await this.db.transaction(async (tx) => {
      ...
      if (grantIds.length > 0) {
        await tx
          ...
          .where(
            or(
              inArray(oidcRecords.grantId, grantIds),
              and(eq(oidcRecords.model, 'Grant'), inArray(oidcRecords.id, grantIds)),
            ),
          );
      }
      if (session.oidcUid) {
