...
export class ClientService {
  ...
  private validate(input: ClientInput): void {
    if (!input.name.trim())
      throw new AppError(400, 'CLIENT_NAME_REQUIRED', 'Client name is required');
  ...
  async create(input: ClientInput): Promise<CreatedClient> {
    ...
    await this.db.transaction(async (tx) => {
      ...
      await tx
        ...
        .values(input.redirectUris.map((uri) => ({ clientId: id, uri })));
      if (input.postLogoutRedirectUris.length > 0) {
        await tx
          .insert(oauthClientPostLogoutRedirectUris)
          .values(input.postLogoutRedirectUris.map((uri) => ({ clientId: id, uri })));
      }
      await tx.insert(oidcRecords).values({
        ...
        payload: clientMetadata(
          id,
          clientId,
          { ...input, accessTokenAudience: audience },
          secretHash,
        ),
      });
    });
  ...
  async setEnabled(clientId: string, enabled: boolean): Promise<void> {
    ...
    await this.db.transaction(async (tx) => {
      await tx
        ...
        .where(eq(oauthClients.id, record.id));
      const [oidc] = await tx
      ...
      if (oidc) {
        await tx
          .update(oidcRecords)
          .set({ payload: { ...oidc.payload, y_auth_enabled: enabled } })
          .where(and(eq(oidcRecords.model, 'Client'), eq(oidcRecords.id, clientId)));
      }
    });
  }
