...
export async function buildApp(environment: NodeJS.ProcessEnv = process.env) {
  ...
  await app.register(helmet, {
    ...
    contentSecurityPolicy: {
      directives: {
        ...
        scriptSrc: [
          "'self'",
          ...(config.TURNSTILE_SITE_KEY ? ['https://challenges.cloudflare.com'] : []),
        ],
  ...
  await registerAdminRoutes(app, { db, clients, sessions, audit, config, redisPing });
  await registerOidcRoutes(app, { provider, auth, sessions, policies, limiter, audit, config });
  ...
  if (existsSync(join(webRoot, 'index.html'))) {
    ...
    app.get('/', async () => ({ name: 'Y.auth', version: '0.1.0', ui: 'Run npm run dev:web' }));
  }
