...
const clients = definitions.map(([name, key, clientId]) => ({
  ...
  manager: new UserManager({
    ...
    userStore: new WebStorageStateStore({ store: sessionStorage, prefix: `e2e:${key}:user:` }),
    stateStore: new WebStorageStateStore({ store: sessionStorage, prefix: `e2e:${key}:state:` }),
...
async function api<T>(user: User, path: string, init?: RequestInit): Promise<T> {
  const response = await fetch(`${issuer}${path}`, {
    ...init,
    headers: { Authorization: `Bearer ${user.access_token}`, ...init?.headers },
  });
    ...
    if (user && !user.expired) {
      ...
      section.append(
        ...
        button('View sessions', async () => {
          const result = await api<{ items: Array<{ lastSeenAt: string }> }>(
            user,
            '/api/v1/me/sessions',
          );
        ...
        button('Logout everywhere', async () => {
          ...
          await api(user, '/api/v1/me/logout-everywhere', { method: 'POST' });
